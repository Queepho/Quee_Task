...
Welcome(){
	...
	if [ $Target == 101 ]
	then
		cp ~/work/Quee/Quee_SR.sh ./Quee_SR.sh
		echo "请输入执行分配CPU核心数目"
		read CORE
		...
		then
				...
				then
					...
					./Quee_SR.sh $CORE $TASK " > ./Q_____SR.slurm
				else
					...
					./Quee_SR.sh $CORE Quee_Task " > ./Q_____SR.slurm
				fi
	...
	then
		cp ~/work/Quee/Quee_SR.sh ./Quee_SR.sh
		cp ~/work/Quee/Quee_SCF.sh ./Quee_SCF.sh
		...
		then
				...
				then
					...
					./Quee_SR_SCF.sh $CORE $TASK " > ./Q_SR_SCF.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_SR_SCF.sh $CORE Quee_Task " > ./Q_SR_SCF.slurm
				fi
	...
	then
		cp ~/work/Quee/Quee_SR.sh ./Quee_SR.sh
		cp ~/work/Quee/Quee_SCF.sh ./Quee_SCF.sh
		cp ~/work/Quee/Quee_BS.sh ./Quee_BS.sh
		cat ./Quee_SCF.sh >> ./Quee_SR.sh
		...
		then
				...
				then
					...
					./Quee_SR_SCF_BS.sh $CORE $TASK" > ./Q_R_F_BS.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_SR_SCF_BS.sh $CORE Quee_TASK " > ./Q_R_F_BS.slurm
				fi
	...
	then
		...
		cp ~/work/Quee/Quee_SCF.sh ./Quee_SCF.sh
		cp ~/work/Quee/Quee_BS.sh ./Quee_BS.sh
		cp ~/work/Quee/Quee_DOS.sh ./Quee_DOS.sh
		cat ./Quee_SCF.sh >> ./Quee_SR.sh
		cat ./Quee_BS.sh >> ./Quee_SR.sh
		...
		then
				...
				then
					...
					./Quee_SR_SCF_BS_DOS.sh $CORE $TASK" > ./Q_R--DOS.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_SR_SCF_BS_DOS.sh $CORE Quee_Task" > ./Q_R--DOS.slurm
				fi
	...
	then
		...
		then
				...
				then
					...
					./Quee_SCF.sh $CORE $TASK" > ./Q____SCF.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_SCF.sh $CORE Quee_Task" > ./Q____SCF.slurm
	...
	then
		...
		then	
				...
				then
					...
					./Quee_BS.sh $CORE $TASK" > ./Q_____BS.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_BS.sh $CORE Quee_Task" > ./Q_____BS.slurm
				fi
	...
	then
		...
		then	
				...
				then
					...
					./Quee_DOS.sh $CORE $TASK" > ./Q____DOS.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_DOS.sh $CORE Quee_Task" > ./Q____DOS.slurm
				fi
	...
	then
		cp ~/work/Quee/Quee_BS_PNG.sh ./Quee_BS_PNG.sh
		echo "请输入执行分配CPU核心数目"
		read CORE
		if [ $(echo $CORE | sed 's/[0-9]*/ /g')=" " ]
		then	
				...
				then
					...
					./Quee_BS_PNG.sh $CORE $TASK" > ./Q__BSPNG.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_BS_PNG.sh $CORE Quee_Task" > ./Q__BSPNG.slurm
				fi
	...
	then
		cp ~/work/Quee/Quee_LDOS_PNG.sh ./Quee_LDOS_PNG.sh
		echo "请输入执行分配CPU核心数目"
		read CORE
		if [ $(echo $CORE | sed 's/[0-9]*/ /g')=" " ]
		then	
				...
				then
					...
					./Quee_LDOS_PNG.sh $CORE $TASK" > ./Q_DOSPNG.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_LDOS_PNG.sh $CORE Quee_Task" > ./Q_DOSPNG.slurm
				fi
	...
	then
		cp ~/work/Quee/Quee_Wan.sh ./Quee_Wan.sh
		echo "请输入执行分配CPU核心数目"
		read CORE
		if [ $(echo $CORE | sed 's/[0-9]*/ /g')=" " ]
		then	
				...
				then
					...
					./Quee_BS.sh $CORE $TASK" > ./Q_W_NSCF.slurm
				else
					echo "未自定义任务名称，设为默认Quee_Task"
					...
					./Quee_BS.sh $CORE Quee_Task" > ./Q_W_NSCF.slurm
				fi
...
Welcome_311(){
	...
	then
		...
		└────────────────────────────────────────"
			...
			WhereIsBS_dpi=$(awk '/BS_dpi/{print NR}' ~/work/Quee/Quee.config)
			sed -i "${WhereIsBS_dpi}s/$BS_dpi/$BS_dpi_new/1" ~/work/Quee/Quee.config
			echo "
...
Welcome_312(){
	...
	then
		...
		└────────────────────────────────────────"
			...
			WhereIsDOS_dpi=$(awk '/DOS_dpi/{print NR}' ~/work/Quee/Quee.config)
			sed -i "${WhereIsDOS_dpi}s/$DOS_dpi/$DOS_dpi_new/1" ~/work/Quee/Quee.config
			echo "
...
Welcome_313(){
	...
	then
		...
		└────────────────────────────────────────"
			...
			WhereIsDOS_x_min=$(awk '/DOS_x_min/{print NR}' ~/work/Quee/Quee.config)
			sed -i "${WhereIsDOS_x_min}s/$DOS_x_min/$DOS_x_min_new/1" ~/work/Quee/Quee.config
			echo "
...
Welcome_314(){
	...
	then
		...
		└────────────────────────────────────────"
			...
			WhereIsDOS_x_max=$(awk '/DOS_x_max/{print NR}' ~/work/Quee/Quee.config)
			sed -i "${WhereIsDOS_x_max}s/$DOS_x_max/$DOS_x_max_new/1" ~/work/Quee/Quee.config
			echo "
...
Welcome_315(){
	...
	then
		...
		then
			WhereIsDOS_y_min=$(awk '/DOS_y_min/{print NR}' ~/work/Quee/Quee.config)
			sed -i "${WhereIsDOS_y_min}s/$DOS_y_min/0/1" ~/work/Quee/Quee.config
			echo "
		│已设置为0,自动返回上一级
		└────────────────────────────────────────"
			sleep 1
			...
			WhereIsDOS_y_min=$(awk '/DOS_y_min/{print NR}' ~/work/Quee/Quee.config)
			IsDash=$(sed -n "${WhereIsDOS_y_min},${WhereIsDOS_y_min}p" ~/work/Quee/Quee.config | grep '-')
			if [ ! -n "$IsDash" ]
			then
				sed -i "${WhereIsDOS_y_min}s/$DOS_y_min/-$DOS_y_min_new/1" ~/work/Quee/Quee.config
				echo "
...
Welcome_316(){
	...
	then
		...
		└────────────────────────────────────────"
			...
			WhereIsDOS_y_max=$(awk '/DOS_y_max/{print NR}' ~/work/Quee/Quee.config)
			sed -i "${WhereIsDOS_y_max}s/$DOS_y_max/$DOS_y_max_new/1" ~/work/Quee/Quee.config
			echo "
...
Welcome_411(){
	cp -n ~/work/Quee/Quee_Wan.config ~/work/Quee/Quee_Wan_temp.config
	PATH_="/home/$partition/work/Quee/Quee_Wan_temp.config"
